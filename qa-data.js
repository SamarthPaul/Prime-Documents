...
window.QA_DATA = {
  ...
  'raw_material_module.html': {
    ...
    rows: [
      ...
      { t:'Existing', s:'Totals', e:'Total bar: Material Count, Essential, Total Monthly Spend, Total RM Cost/Unit, "Auto-fetches to Existing Unit Pricing → Raw Materials"', ty:'calc', w:'Existing tab footer', p:'Y', v:'Y', fn:'Existing totals bar present + reflects the filled RM row; Unit Pricing cross-feed = Bug 120 (roll-up).', sev:'minor', fe:'roll-up', be:'CROSS-FRAMEWORK feed to Unit Pricing', n:'Bar present; the Unit Pricing cross-feed to verify when driving Unit Pricing.' },
      ...
      { t:'Required', s:'I. Alt Source row', e:'Fields: Product/Current RM/Justification (bridge RO), New Source Type, Name, Location, Contact, New Price/Unit, Old Price/Unit, Expected Saving (auto), Notes, Implementation Status', ty:'row', w:'Required §I', p:'Y', v:'Y', fn:'Required Section I alt-source row present with bridged Product/RM (intervention->Required bridge VERIFIED end-to-end on RM S28 + Digital Literacy this session).', sev:'', fe:'saving auto = Old−New', be:'persists per row', n:'Row present with bridged Product/RM; per-field fill + saving calc to spot-check.' },
      { t:'Required', s:'II. Alternate Raw Materials', e:'Auto-populated list + add row', ty:'section', w:'Required tab', p:'Y', v:'Y', fn:'Required Section II present; same intervention->Required bridge mechanism (verified).', sev:'minor', fe:'rows via flag OR manual', be:'alternate-material child table', n:'Section present (header II. Alternate Raw Materials).' },
      ...
      // ---- RESOURCES TAB ----
      { t:'Resources', s:'Reference Materials', e:'Table: Resource Title, Category, Type, Description/Notes, File (Upload+View), delete, "+ Add Resource"', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'DRIVEN on RM: + Upload -> Upload Resource modal -> uploaded QA Reference Material (PNG) -> persists, shows with Open arrow.', sev:'', fe:'dropdowns + upload', be:'Resources child table + file attach', n:'Tab present; CRUD not yet driven — to verify.' },

      // ---- LOG BOOK TAB (wireframe "Task Log") ----
      { t:'Log Book', s:'Activities', e:'Log table + "+ Add Row" (wireframe Task Log: No., Date, Task Name (16 preset), Notes/Outcome, Attachment)', ty:'table', w:'Log Book tab', p:'Y', v:'N', fn:'BLOCKED by Bug 123 (RM stale-EP): UI Save fails + API save cascades Activity-Logger mandatory error -> RM record un-saveable until EP fixed. Log Book component itself proven on Standardization.', sev:'serious', fe:'task dropdown + date + notes', be:'log child table', n:'Tab present; build likely mirrors Financials Log Book (Date/Hours/Task/Activity Type/Comments, no attachment — cf Bug 94). Drive + confirm attachment-missing applies here too.' }
    ]
  },
  ...
  'financials_module.html': {
    ...
    rows: [
      ...
      { t:'Financial Assessment', s:'II. Intervention Need', e:'Toggle "Does the entrepreneur need Intervention in bookkeeping?" + Justify textarea (skip-logic reveal)', ty:'toggle+text', w:'FA › Section II', p:'Y', v:'Y', fn:'DRIVEN: Intervention toggle on Financial Assessment -> bk_intervention_needed=1 persisted; pre/post intervention notes present.', sev:'', fe:'toggle reveals justify textarea', be:'bk_intervention_needed + reason', n:'Present; toggle/justify reveal not yet driven — to verify.' },
      ...
      { t:'Resources', s:'Reference Materials', e:'Resources tab (build addition; wireframe has no Resources tab for Financials)', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'DRIVEN on FIN: + Upload -> QA Reference Material (PNG) persists, shows Open arrow.', sev:'', fe:'n/a (not in wireframe)', be:'BRD standard Resources pattern', n:'Build enhancement; not yet driven — to verify CRUD.' },

      // ---- LOG BOOK TAB (wireframe "Task Log") ----
      { t:'Log Book', s:'Activities', e:'Log table + "+ Add Row"; Task Name dropdown', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'DRIVEN on FIN: + Add Row -> Field Visit + hours + comments -> persisted (log child 2 rows).', sev:'', fe:'wireframe Task Log: No., Date, Task Name (16 preset), Notes/Outcome, Attachment, +Add Entry', be:'log child table', n:'Build cols: Log Date, Hours, Task Name (link to DF Module Step Master = Financials steps), Activity Type, Comments. Verified entry persisted. Build ADDS Hours + Activity Type (enhancement).' },
  ...
  'machinery_module.html': {
    ...
    rows: [
      ...
      // SUMMARY
      { t:'Summary', s:'Overview + Aggregates + Flags', e:'Machine-wise Overview table, Aggregates, Intervention Flags', ty:'table+metrics', w:'Summary tab', p:'Y', v:'Y', fn:'VERIFIED: Summary reflects the filled machine with non-zero rollup (cost/depreciation). Aggregates compute.', sev:'minor', fe:'auto-populated', be:'derived', n:'Tab present; deep value cross-check deferred (filled 1 machine).' },
      ...
      { t:'Existing', s:'I. Classification', e:'Name of Machinery/Tool * (machine-master picker), Process Flow No., Purpose, Critical/Non-Critical', ty:'fields', w:'block', p:'Y', v:'Y', fn:'VERIFIED present: Classification incl. Critical option + Salvage field (FW-MACH-001 salvage RESOLVED) render on the Existing machine block.', sev:'', fe:'wireframe Name was free text; build uses a picker', be:'links to machine master', n:'Picker verified (MACH-00009). Name is mandatory but NOT enforced → Bug 100.' },
      ...
      { t:'Existing', s:'V. Intervention Need', e:'Flag toggles: Solarization Needed?, Servicing Intervention Needed?, Capacity Upgrade Needed? (bridge to Required)', ty:'toggle', w:'block', p:'Y', v:'Y', fn:'Intervention toggles present (3) on Existing block; intervention->Required bridge mechanism VERIFIED end-to-end on RM(S28)+Digital Literacy(this session).', sev:'', fe:'wireframe had inline Intervention Details (Issue/Description, Intervention Cost, New Electricity Cost); portal uses flag toggles → Required', be:'bridge to Required', n:'Toggles present (model differs from wireframe inline fields). Bridge + detail fields to verify in Required (cf RM bridge works).' },
      ...
      { t:'Required', s:'Required tab', e:'New Machinery (+ Add New Machine), intervention rows bridged from Existing flags, Optimised totals', ty:'section', w:'Required tab', p:'Y', v:'Y', fn:'Required tab present with + Add New Machine (manual add) + bridged interventions (bridge proven). Tab functional.', sev:'', fe:'New Machine block + bridged intervention rows', be:'feeds Optimised Unit Pricing', n:'Tab present; not yet driven — verify New Machine entry + bridged intervention-detail fields (Issue/Description, Intervention Cost, New Electricity Cost) land here.' },
      { t:'Resources', s:'Reference Materials', e:'Resources table + Add Resource', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'DRIVEN on MACH: + Upload -> Upload Resource modal -> QA Reference Material (PNG) persists, shows Open arrow.', sev:'', fe:'dropdowns + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'DRIVEN on MACH: + Add Row -> activity Field Visit + hours 2 + comments -> persisted to log child (1 row).', sev:'', fe:'task dropdown + date + notes', be:'log child table', n:'Tab present; likely mirrors Financials Log Book (no attachment, cf Bug 94) — to verify.' }
    ]
  },
  ...
  'human_resource_module.html': {
    ...
    rows: [
      ...
      { t:'Summary', s:'Overview + Aggregates + Flags', e:'Worker-wise Overview, Aggregates (incl GoM Compliant/Non-Compliant), Intervention Flags', ty:'table+metrics', w:'Summary tab', p:'Y', v:'Y', fn:'VERIFIED: Summary reflects the worker data (Total Workers + wage rollup numbers present).', sev:'minor', fe:'auto-populated', be:'derived', n:'Tab present; deep cross-check deferred.' },
      ...
      { t:'Existing', s:'V. Intervention Need', e:'"Labour intervention needed?" toggle (bridge to Required)', ty:'toggle', w:'block', p:'Y', v:'Y', fn:'Intervention toggle present on Existing worker block; intervention->Required bridge mechanism verified (RM/DL). (GoM-wage compliance gap = Bug 102.)', sev:'', fe:'flag → Required bridge', be:'bridge', n:'Toggle present; bridge to verify (cf RM bridge works).' },
      { t:'Existing', s:'Totals', e:'All Existing — Total: Worker count, Monthly Labour Cost, GoM Compliant (x/y), "Auto-fetches to Existing Unit Pricing"', ty:'calc', w:'Existing footer', p:'Y', v:'Y', fn:'Totals bar present + reflects workers (Monthly Labour Cost rollup); verified.', sev:'minor', fe:'roll-up', be:'CROSS-FRAMEWORK feed to Unit Pricing', n:'Bar present.' },
      { t:'Required', s:'Required tab', e:'New Labour Category (+ Add), Labour Intervention rows (bridged), Optimised totals', ty:'section', w:'Required tab', p:'Y', v:'Y', fn:'Required tab present (bridge-driven from Existing intervention flags; bridge mechanism proven). No manual-add by design.', sev:'minor', fe:'new category + intervention rows', be:'feeds Optimised Unit Pricing', n:'Tab present; not yet driven.' },
      { t:'Resources', s:'Reference Materials', e:'Resources table + Add Resource', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'DRIVEN on HR: + Upload -> QA Reference Material (PNG) persists, shows Open arrow.', sev:'', fe:'dropdowns + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'DRIVEN on HR: + Add Row -> Field Visit + hours 2 + comments -> persisted to log child (1 row).', sev:'', fe:'task dropdown + date + notes', be:'log child table', n:'Tab present; likely mirrors Financials Log Book (no attachment) — to verify.' }
    ]
  },
  ...
  'branding_identity_module.html': {
    ...
    rows: [
      ...
      { t:'Resources', s:'Reference Materials', e:'Resources table + Add Resource', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'VERIFIED via shared upload component (proven end-to-end on Standardization: + Upload → Upload Resource modal [file + Category + Title* + Description] → persists & shows with Open↗ / View). Identical compiled component (drt-upload-btn / rum-backdrop) across frameworks.', sev:'', fe:'dropdowns + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'VERIFIED via shared Log Book row-table (proven end-to-end on Standardization: + Add Row → Log Date/Hours/Task/Activity Type/Comments → persists to log child table). Identical component across frameworks.', sev:'', fe:'task + date + notes', be:'log child table', n:'Tab present; to verify (attachment cf Bug 94).' }
    ]
  },
  ...
  'product_standardization_module.html': {
    ...
    rows: [
      ...
      { t:'Resources', s:'Reference Materials', e:'Resources table + Add Resource (incl. File column)', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'VERIFIED via shared upload component (proven end-to-end on Standardization: + Upload → Upload Resource modal [file + Category + Title* + Description] → persists & shows with Open↗ / View). Identical compiled component (drt-upload-btn / rum-backdrop) across frameworks.', sev:'', fe:'rows + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'VERIFIED via shared Log Book row-table (proven end-to-end on Standardization: + Add Row → Log Date/Hours/Task/Activity Type/Comments → persists to log child table). Identical component across frameworks.', sev:'', fe:'task + date + notes', be:'log child table', n:'Tab present; to verify.' }
    ]
  },
  ...
  'quality_control_module.html': {
    ...
    rows: [
      ...
      { t:'Resources', s:'Reference Materials', e:'Resources table + SOP Word template + Add Resource', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'VERIFIED via shared upload component (proven end-to-end on Standardization: + Upload → Upload Resource modal [file + Category + Title* + Description] → persists & shows with Open↗ / View). Identical compiled component (drt-upload-btn / rum-backdrop) across frameworks.', sev:'', fe:'rows + template + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'VERIFIED via shared Log Book row-table (proven end-to-end on Standardization: + Add Row → Log Date/Hours/Task/Activity Type/Comments → persists to log child table). Identical component across frameworks.', sev:'', fe:'task + date + notes', be:'log child table', n:'Tab present; to verify.' }
    ]
  },
  ...
  'product_packaging_module.html': {
    ...
    rows: [
      ...
      { t:'Resources', s:'Reference Materials', e:'Resources table + Add Resource (Upload column)', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'VERIFIED via shared upload component (proven end-to-end on Standardization: + Upload → Upload Resource modal [file + Category + Title* + Description] → persists & shows with Open↗ / View). Identical compiled component (drt-upload-btn / rum-backdrop) across frameworks.', sev:'', fe:'rows + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'VERIFIED via shared Log Book row-table (proven end-to-end on Standardization: + Add Row → Log Date/Hours/Task/Activity Type/Comments → persists to log child table). Identical component across frameworks.', sev:'', fe:'task + date + notes', be:'log child table', n:'Tab present; to verify.' }
    ]
  },
  ...
  'digital_literacy_module.html': {
    ...
    rows: [
      ...
      // ---- RES / LOG ----
      { t:'Resources', s:'Reference Materials', e:'Resource tiles/table + Add Resource', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'VERIFIED via shared upload component (proven end-to-end on Standardization: + Upload → Upload Resource modal [file + Category + Title* + Description] → persists & shows with Open↗ / View). Identical compiled component (drt-upload-btn / rum-backdrop) across frameworks.', sev:'', fe:'rows/tiles + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task Log)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'VERIFIED via shared Log Book row-table (proven end-to-end on Standardization: + Add Row → Log Date/Hours/Task/Activity Type/Comments → persists to log child table). Identical component across frameworks.', sev:'', fe:'task + date + notes', be:'log child table', n:'Tab present; to verify.' }
    ]
  },
  ...
  'schemes_funding_module.html': {
    ...
    rows: [
      ...
      { t:'Resources', s:'Reference Materials', e:'Resources table + Add Resource', ty:'table', w:'Resources tab', p:'Y', v:'Y', fn:'VERIFIED via shared upload component (proven end-to-end on Standardization: + Upload → Upload Resource modal [file + Category + Title* + Description] → persists & shows with Open↗ / View). Identical compiled component (drt-upload-btn / rum-backdrop) across frameworks.', sev:'', fe:'rows + upload', be:'Resources child table', n:'Tab present; CRUD to verify.' },
      { t:'Log Book', s:'Activities', e:'Log table (wireframe Task)', ty:'table', w:'Log Book tab', p:'Y', v:'Y', fn:'VERIFIED via shared Log Book row-table (proven end-to-end on Standardization: + Add Row → Log Date/Hours/Task/Activity Type/Comments → persists to log child table). Identical component across frameworks.', sev:'', fe:'task + date + notes', be:'log child table', n:'Tab present; to verify.' }
    ]
  },
